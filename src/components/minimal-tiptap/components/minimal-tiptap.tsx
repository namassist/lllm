  ...
  (
    ...
    const editor = useEditor({
      ...
      editorProps: {
        attributes: {
          class:
            "prose mx-auto focus:outline-none max-w-none prose-stone dark:prose-invert minimal-editor",
        },
      },
      onUpdate: (props) => {
    ...
    return (
      <div
        className={cn(
          "flex h-auto min-h-72 w-full flex-col rounded-md border shadow-sm border-primary",
          className
        )}
        {...props}
        ref={ref}
      >
        {editor && (
          <>
            ...
            {!disabled && <ImageBubbleMenu editor={editor} />}
...
const Toolbar = ({ editor }: { editor: TiptapEditor }) => {
  return (
    <div className="border-b-2 border-primary/50 p-2">
